# Better Commerce Continuation Brief
...
1. `docs/plans/continuation.md` — this handoff;
2. `docs/README.md` — documentation map and lifecycle;
3. `docs/adr/0001-platform-principles.md` — accepted platform philosophy;
4. `docs/adr/0002-monorepo.md` — accepted repository and workspace boundaries;
5. `docs/adr/0003-backend.md` — accepted and frozen backend module architecture;
6. `docs/adr/0004-commerce-model.md` — accepted commerce capability model;
7. `docs/contracts/authorization.md` — authorization behavior that must not regress;
8. `docs/runbooks/local-development.md` and `README.md` — current workflows.

Do not redesign ADR-0001 through ADR-0004 during the next decision or
implementation wave. ADR-0003 is explicitly frozen; normative changes require
its amendment process. Other accepted ADR changes require an amendment or a
superseding decision rather than silent edits.
...
### ADR-0003 Agent 2 — Identity consolidation

Completed:

- consolidated Auth, Users, and Session under `apps/api/src/modules/identity`;
- introduced `IdentityModule` and a narrow Module Public Contract;
- kept entities, repositories, credentials, verification tokens, and TypeORM
  implementation private;
- preserved every existing authentication, email-verification, and session
  behavior;
- passed direct and uncached root build/type-check/lint/unit gates and all
  environment-independent E2E tests.

### ADR-0003 Agent 3 — Authorization authority and transactions

Completed:

- moved Authorization under `apps/api/src/modules/authorization`;
- removed business-level Authorization access to Identity entities and
  repositories;
- introduced an opaque transaction context backed by an infrastructure-only
  TypeORM manager mapping;
- preserved atomic staff/role/audit writes and Identity `authVersion` changes;
- removed cross-module ORM traversal while retaining one private,
  non-loading/non-cascading foreign-key metadata exception;
- added automated module-boundary checks;
- passed 19 unit suites/66 tests, eight architecture checks, and all five
  PostgreSQL/Redis-backed E2E suites/27 tests;
- proved rollback across Authorization/Identity and concurrent last-owner
  serialization with PostgreSQL.

Docker Desktop, PostgreSQL, and Redis were available for Agent 3's final
verification. The user accepted ADR-0003 implementation and waived the optional
independent Agent 4 pass. Live process/OpenAPI/real-user bootstrap smoke checks
remain release-readiness checks rather than a known code defect.

## Exact next objective

ADR-0004 — Commerce Domain Model is accepted. The next objective is to review
and write ADR-0005 — Catalog, Products, and Variants.

ADR-0005 must remain inside ADR-0004's accepted boundaries:

- Catalog owns Products, Variants, merchandising, and fulfillment
  classification;
- every purchasable Product has at least one Variant;
- Pricing and Inventory remain separate authorities;
- no universal Sellable hierarchy or speculative future capability is created;
- the ADR is reviewed and accepted before Catalog implementation starts.

After ADR-0005, continue with ADR-0006 Orders, ADR-0007 Pricing, and ADR-0008
Inventory. Implementation may use a narrow vertical slice only after the
relevant focused ADRs are accepted.
...
> Continue Better Commerce from `docs/plans/continuation.md`. Read it,
> `docs/README.md`, and ADR-0001 through ADR-0004 completely, then inspect the
> actual Git state. Preserve all existing changes. ADR-0002 and ADR-0003
> implementation are complete, and ADR-0004 is accepted. Work next on the
> proposed ADR-0005 Catalog/Product/Variant decision. Do not implement commerce,
> Admin, storefront, SDK, registry, or kernel work before its relevant ADR is
> accepted.
